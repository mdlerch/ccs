wwwwlasso <- function(x, y, w)
{

    # adaptive lasso from lars with BIC stopping rule this one uses the 'known
    # variance' version of BIC with RSS/(full model mse) must use a recent version of
    # R so that normalize=FALSE can be used in lars
    ...
    # get min BIC bic=log(n)*object$df+n*log(as.vector(object$RSS)/n) # rss/n version
    ...
    # if (st > 0) 
    #     x.ind <- as.vector(which(coeff != 0)) else x.ind <- 0
    # intercept <- as.numeric(mean(y) - meanx %*% coeff)
    # return(list(fit = fit, st = st, mse = mse, x.ind = x.ind, coeff = coeff, intercept = intercept, 
    #     object = object, bic2 = bic2, step.bic2 = step.bic2))
    object
}
